...
console.log(videoPlaySectionData);
...
function createPlaylistCard(obj, pos) {
    ...
    mainDiv.onclick = function(e) {
        ...
        loadVideo(obj.id);
        console.log(obj.id + 'in card fun');
...
for (i = 0; i < playlistData.length; i++) {
    playlistWrapper.appendChild(createPlaylistCard(playlistData[i]), i);
}
...
function loadVideo(id) {
    ...
    document.getElementById("views-count").textContent =
        videoPlaySectionData[id - 1].views;
    document.getElementById("video-title").textContent =
        videoPlaySectionData[id - 1].title;
    document.getElementById("video-description").textContent =
        videoPlaySectionData[id - 1].description;
}

function removeClass() {
    for (i = 0; i < 10; i++) {
        document.getElementById("card" + playlistData[i].id).classList.remove("active-card");
    }
}